...
pub trait IngestEnclave: ReportableEnclave {
    ...
    /// The public key corresponding to this value is also returned to help catch races.
    ...
    /// Set the private key of the enclave, encrypted by the peer for this enclave
    fn set_ingress_private_key(
        &self,
        msg: EnclaveMessage<PeerSession>,
    ) -> Result<(RistrettoPublic, SealedIngestKey)>;

    /// Retrieve the current KexRngPubkey for the enclave. This corresponds to the egress key.
    fn get_kex_rng_pubkey(&self) -> Result<KexRngPubkey>;

    /// Consume all the transactions and emit corresponding rows for the recovery database

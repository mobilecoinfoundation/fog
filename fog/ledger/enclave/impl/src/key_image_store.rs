...
use alloc::boxed::Box;
use fog_types::view::{KeyImageResult, KeyImageResultCode};
...
impl<OSC: ORAMStorageCreator<StorageDataSize, StorageMetaSize>> KeyImageStore<OSC> {
    ...
    pub fn find_record(&mut self, search_key: &[u8]) -> KeyImageResult {
        let mut result = KeyImageResult {
            search_key: search_key.to_vec(),
            result_code: KeyImageResultCode::InternalError as u32,
            ciphertext: vec![0u8; ValueSize::USIZE - 1 - self.last_ciphertext_size_byte as usize],
        };

        // Early return for bad search key
        if search_key.len() != KeySize::USIZE {
            result.result_code = KeyImageResultCode::BadSearchKey as u32;
            return result;
        }
        ...
        // OMAP_FOUND -> KeyImageResultCode::Found
        // OMAP_NOT_FOUND -> KeyImageResultCode::NotFound
        // OMAP_INVALID_KEY -> KeyImageResultCode::BadSearchKey
        // Other -> KeyImageResultCode::InternalError, debug_assert!(false)
        {
            let oram_result_code = self.omap.read(&key, &mut value);
            result.result_code.cmov(
                oram_result_code.ct_eq(&OMAP_FOUND),
                &(KeyImageResultCode::Found as u32),
            );
            result.result_code.cmov(
                oram_result_code.ct_eq(&OMAP_NOT_FOUND),
                &(KeyImageResultCode::NotFound as u32),
            );
            result.result_code.cmov(
                oram_result_code.ct_eq(&OMAP_INVALID_KEY),
                &(KeyImageResultCode::BadSearchKey as u32),
            );

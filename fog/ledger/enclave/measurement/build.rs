...
//! Build script for the ledger_enclave_measurement crate.
...
const LEDGER_ENCLAVE_SECURITY_VERSION: u16 = 1;
...
fn main() {
    ...
    let mut builder = Builder::new(
        ...
        SGX_VERSION,
        LEDGER_ENCLAVE_NAME,
        LEDGER_ENCLAVE_DIR.as_ref(),
    )
    .expect("Could not construct builder");

    rerun_if_env_changed!("LEDGER_ENCLAVE_CSS");
    if let Ok(value) = var("LEDGER_ENCLAVE_CSS") {
        builder.css(PathBuf::from(&value));
    }

    rerun_if_env_changed!("LEDGER_ENCLAVE_UNSIGNED");
    if let Ok(value) = var("LEDGER_ENCLAVE_UNSIGNED") {
        builder.unsigned_enclave(PathBuf::from(&value));
    }

    rerun_if_env_changed!("LEDGER_ENCLAVE_SIGNED");
    if let Ok(value) = var("LEDGER_ENCLAVE_SIGNED") {
        builder.signed_enclave(PathBuf::from(&value));
    }

    rerun_if_env_changed!("LEDGER_ENCLAVE_LDS");
    if let Ok(value) = var("LEDGER_ENCLAVE_LDS") {
        builder.lds(PathBuf::from(&value));
    }

    rerun_if_env_changed!("LEDGER_ENCLAVE_PRIVKEY");
    if let Ok(value) = var("LEDGER_ENCLAVE_PRIVKEY") {
        builder.privkey(PathBuf::from(&value));
    }

    rerun_if_env_changed!("LEDGER_ENCLAVE_GENDATA");
    rerun_if_env_changed!("LEDGER_ENCLAVE_PUBKEY");
    rerun_if_env_changed!("LEDGER_ENCLAVE_SIGNATURE");
    if let Ok(gendata) = var("LEDGER_ENCLAVE_GENDATA") {
        if let Ok(pubkey) = var("LEDGER_ENCLAVE_PUBKEY") {
            if let Ok(signature) = var("LEDGER_ENCLAVE_SIGNATURE") {
    ...
    let _sig = builder
        .build()
        .expect("Failed to extract ledger-enclave signature");
}

...
/// This object keeps track of all TxOut's that are known to be ours, and which have been spent.
/// It allows to check the current balance of the account, and to find suitable inputs for a new transaction.
///
/// Given network connections to fog view and fog ledger, it can poll for new balance data.
#[derive(Clone)]
pub struct CachedTxData {
    /// The UserRngSet. This is state related to conducting the fog-view protocol, which produces TxOut objects.
    /// The rng_set also exposes the `num_blocks()` value which tells us to what block we are guaranteed to have
    /// all of the user's TxOuts
    rng_set: UserRngSet,
    /// The collection of OwnedTxOuts, keyed by the global index.
    ///
    /// These are TxOuts that we have found, that we know are ours, by view-key scanning them.
    /// These TxOuts come either from fog-view, or from downloading blocks from untrusted fog-ledger,
    /// or possibly another source.
    /// These TxOuts may already have been spent or not, determined by `status` field.
    owned_tx_outs: BTreeMap<u64, OwnedTxOut>,
    /// Represents how fresh our information about unspent key images is.
    ///
    /// Invariant:
    /// self.key_image_data_completness is less or equal to the block count number
    /// for any owned transaction with NotSpent(block_count) status
    ///
    /// When computing balances, we take the min of this with the rng_set num_blocks value and try to target that time for
    /// computing the balance, because then we know we have all the transactions up to that point, and we know that we have
    /// complete information about whether those transactions were spent up to that point.
...
impl CachedTxData {
    ...
    /// This is the minimum of all the sources of data that we have from different servers.
    ///
    /// Note: Missed Blocks are not implemented in the paykit at this revision,
    /// the implementation would have to keep track of if there are any blocks that
    /// fog view told us about that we have to download and scan, and then we would have
    /// to track if we did that. Then this `min` expression would also take the min of
    /// outstanding missed blocks.
    ...
    /// that we are aware of, and helps to ensure that when sampling RingCT mixins,
    /// we make requests that are in-bounds.
    ...
    /// where balance is the correct balance when the ledger has exactly block_count blocks
    #[allow(clippy::nonminimal_bool)]
    pub fn get_balance(&self) -> (u64, BlockCount) {
        ...
        // we want to compute a balance that was definitely consistent with some specific
        ...
        // This can happen if we only have TxOut's up to block 10 but we have key image data up to block 12.
        // Such a transaction is then NOT selected by get_unspent_txos, because we know that building a new
        // transaction using that as an input would fail when we submit.
        //
        // Because of this it is possible that get_balance returns a particular balance, but get_transaction_inputs
        // for that balance subsequently fails with "Insufficient Funds". In this case you should get balance again
        // and try again.
        log::trace!(
            self.logger,
    ...
    ///
    /// This ONLY picks transactions that are part of the most recent balance check,
    /// and are not known to be spent at any time.
    ///
    /// Arguments:
    /// * An amount (u64), with any fee included.
    /// * A maximum number of inputs to include in this transaction.
    ///   The number should be based on dividing the user-provided fee by the base-fee
    ///   according to the scaling.
    ///   This argument will be clamped to mc_transaction_core::constants::MAX_INPUTS.
    ///
    /// Returns:
    /// * A collection of our unspent transactions that add up to at least this value,
    /// * Error::Insufficient if there are insufficient funds
    /// * Error::WalletCompactingNeeded if there are enough funds, but not enough
    ///   large TxOuts to pay for the transaction without going over /the maximum inputs limit.
    ///   This error code includes a recommended size for a self-payment that will consume
    ///   the oldest small transactions (not including fee for self-payment).
    ///   If the oldest small transactions don't add up to at least the fee, then this self-payment
    ///   is not actually economical to perform, and you should wait until the fee is lower.
    ...
    /// and then adds them to the cache, sorting the cache and deduplicating it as well.
                ...
                Ok(otxo) => {
                    ...
                    match otxo.status {
                        ...
                        KeyImageStatus::NotSpent(as_of) => {
                            // It's okay for this number to get smaller, as long as num_blocks doesn't get smaller
                            self.key_image_data_completeness =
    ...
    ///
    /// This is called when doing a balance check
    /// This may be called after poll_fog_for_txos, so that we can ask about any new key images returned by that call
    pub fn poll_fog_for_key_images(
        ...
        // TxOut public keys are enforced to be unique by consensus, but it is technically possible
        // that the key image repeats.
        //
        // Note that only one of the two TxOuts will actually be spendable if this happens,
        // so it might be a good idea to simply ignore the one of lesser value.
        // Then this would be handled in the consume_new_txo_records function.
                            ...
                            Ok(None) => {
                                match &mut otxo.status {
                                    ...
                                    KeyImageStatus::NotSpent(not_spent_as_of) => {
                                        ...
                                        // If the new information older than the old information, don't discard the old information
                                        *not_spent_as_of = max(
                                            *not_spent_as_of,
    ...
    /// Debug print balance information (to help debug a wrong balance computation)
...
/// A status held by a key image -- it can either be spent in a certain block, or known not to be spent as of a certain block.
///
/// The default status, if we have no other information, is that it is not spent as of the block in which it is created.
/// This is because it is impossible to spend a TxOut in the same block that creates it.
...
/// Several fields like key_image and value are cached so that they do not need to be recomputed.
#[derive(Debug, Clone)]
pub struct OwnedTxOut {
    ...
    /// The tx_out that we recovered from the view server, or from view-key scanning a missed block.
    pub tx_out: TxOut,
    /// The value of the TxOut, computed when we matched this tx_out successfully against our account key.
    pub value: u64,
    /// The key image that we computed when matching this tx_out against our account key.
    pub key_image: KeyImage,
    /// The status of the key image, which we learn by querying key image server.
    /// This is either `spent_at(index)` or `not_spent_as_of(count)`.
    pub status: KeyImageStatus,
}

impl OwnedTxOut {
    /// Try to decrypt a TxOutRecord by view-key matching it, producing an OwnedTxOut or an error
    //
    // FIXME: FOG-237 Confirm that this subaddress matched the tx. Should use view_key_matches_output function
    // At time of writing this is not used by mobilecoind or other mobilecoin rust code, but it is used by the swift sdk,
    ...
    pub fn new(rec: TxOutRecord, account_key: &AccountKey) -> StdResult<Self, TxOutMatchingError> {
        ...
        // This is view key scanning part, getting the value fails if view-key scanning fails
...
///
/// The input selection heuristic chooses which of our Txos to use as inputs to meet
/// a target amount, based on their relative ages and values.
///
/// There are several competing concerns:
/// - There is a network-wide maximum number of inputs to a transaction, so possibly not all inputs can be used
/// - The number of inputs may depend on the fee and fee-scaling rule
/// - In cryptonote, older inputs are generally harder to conceal than younger inputs, so we usually want
///   to prefer to spend our older inputs until they get even older
/// - We want to opportunistically compact the user's wallet over time so that wallet-compaction transactions
///   are not usually needed.
/// - If wallet-compaction transactions are needed, we offer guidance as to what is the best way to compact the wallet.
///
/// As a heuristic, we attempt to spend the oldest tx outs that we can
/// while still achieving the required amount and not exceeding the max inputs limit.
/// We stop once we hit the required amount.
///
...
/// * max_inputs: the maximum number of inputs we can use, which the caller may compute based on the fee
///               This is clamped to mc_transaction_core::constants::MAX_INPUTS
///
/// Returns:
/// * The indices to use to build the transaction
/// * Or, an error indicating that there are insufficient funds or that wallet compaction is needed
fn input_selection_heuristic(
    ...
    // the MAX_INPUTS limit. We would also like to favor spending the earlier transactions
    // if at all possible, because we want the wallet to naturally consolidate itself so
    // that the user doesn't typically have to pay fees to compact it.
    //
    // We use a dynamic programming approach:
    //
    // We want to find for every i, the most valuable collection of j transactions
    // from among the i+1 ... of the available list, for values of j from 0 to MAX_INPUTS.
    // Once we have this list, we can then consider the earliest transactions first,
    // and consider if it is possible to include them while achieving the amount.
    //
    // dp_table[i][j] := maximum sum of not more than j transaction values chosen from available[i..]
    //
    // The table can be filled iteratively by working with largest values of i first,
    // and smallest values of j first. For convenience, the matrix has an extra row and column
    // which are initialized to zeroes and never changed, to simplify boundary conditions.
    //
    ...
    let dp_table = {
        ...
        for i in (0..tx_values.len()).rev() {
            // The best value for 0 transactions is 0, so we iterate from j to MAX_INPUTS and set dp_table[i][j+1].
            for j in 0..MAX_INPUTS {
                // The best value for j+1 transactions chosen from available[i..] is obtained either
                // * use the i'th transaction and j transactions from available[(i+1)..], or
                // * don't use the i'th transaction, and use j+1 transactions from available[(i+1)..].
                //
    ...
    if dp_table[0][max_inputs] < amount {
        ...
        // Recommend to make the largest self-payment possible to extend our range the next time we try.
        // If user passed less than 3 inputs, then choose at least three.
        return Err(InputSelectionError::WalletCompactingNeeded(
            dp_table[0][max(max_inputs, 3)],
        ));
    }

    // Now greedily take the earliest transactions that we can while still achieving target amount.
    // result is the collection that we will return, and value is its running value
    ...
    // Correctness:
    // If this assert fails, then earlier assert `result.len() < max_inputs` would have had to fail.
    assert!(result.len() <= max_inputs, "invariant was violated");
    // Correctness:
    // If this assert fails, then the test `dp_table[0][max_inputs] < amount` would have failed,
    // and we would have exited with "wallet compacting needed". This is the point of the dp algorithm.
...
/// fog_sample_paykit::Error cannot do this because grpcio and mc_transaction_std Error's don't
/// implement Eq.

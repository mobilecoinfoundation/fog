...
//!
//! This is not actually intended to be exposed as part of libmobilecoin, because
//! it probably cannot be done synchronously, but it's useful for tests, and it's
...
pub trait FogViewConnection {
    ...
    /// - start_from_user_event_id: Limit user events search to only event ids higher than this
    /// - start_from_block_index: Limit ETxOutRecord search for only tx outs that appeared in or
    ...
    /// This is logic that is expected to be implemented on client-side in paykits
    /// but this at least serves as a reference implementation, abstracting out any
    /// use of grpcio.
    fn poll(
        &mut self,
        ...
        // It's not considered acceptable that one error can cause the whole process fail,
        // instead we get as many hits as we can and return any new txs as well as any errors.
        ...
        // Optimization: If the num_blocks of the view server hasn't changed, or is older,
        // since last time we talked to it, then we don't have any new tx's.
        ...
        loop {
            ...
            // From any non-dead rng, collect "request_multiplier" outputs, without advancing the rng.
            ...
            let resp = match self.request(
                ...
                Ok(resp) => resp,
                Err(err) => {
                    // If there's a connection error it's probly unrecoverable and we should not loop
                    // There are retries in the FogClient class
            ...
            // The new num blocks value is the minimum of all the num_blocks values we got back from
            // the server in all queries we made to it. This is needed so that we can guarantee that
            // if we don't find a transaction for some particular rng output, then it didn't land
            // before num_blocks.
            ...
            // Feed all the new TxOutSearchResult objects into the user_rng_set at once, which is more efficient
            ...
            // Calculate dead rngs
            // Compare current rngs vs. old_rngs, if we did not advance at least (exactly) request_multiplier steps, then we are dead.
            ...
            // Ask for twice as many values from each rng next round, so that we only need log n round trips.
        ...
        // are behind where they were the last time we talked to them (due to load balancer),
        // that doesn't mean our previous balance computation was wrong, it just means
        // we didn't get any new useful information.

...
The `fog_local_network.py` script is meant to assist in running a fog-enabled local network. It relies on `public/tools/local-network/local_network.py` for starting a consensus network, and then add the various fog services on top of it.
...
2) Generating a set of sample keys:
    ```
    cargo run -p mc-util-keyfile --bin sample-keys --release --manifest-path ../public/Cargo.toml -- --num 1000 --fog-report-url 'insecure-fog://localhost:6200'
    ```
...
3) Bootstrap a ledger:
    ```
    cargo run --manifest-path ../public/util/generate-sample-ledger/Cargo.toml --release -- --txs 100
    ```
...
4) Start a local network, for example:
    ...
    ```
    Note that all of the above arguments are identical to the public local_network.py script.
...
5) Distribute coins with usable fog hints:
    ...
    cargo run -p mc-util-keyfile --bin sample-keys --release --manifest-path ../public/Cargo.toml -- --num 10 --output-dir fog_keys --fog-report-url 'insecure-fog://localhost:6200'
...
If you have already built in public and internal, you can skip the build step with `--skip-build`.

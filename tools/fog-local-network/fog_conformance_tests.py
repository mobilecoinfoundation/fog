...
class TestLedger:
    def __init__(self, name, ledger_db_path, watcher_db_path, keys_dir, release):
        ...
        cmd = ' '.join([
            f'cd {self.ledger_db_path} && exec {INTERNAL_PROJECT_DIR}/{target_dir(self.release)}/init_test_ledger',
            f'--keys {keys_dir}',
    ...
    def add_block(self, credits, key_images, fog_pubkey):
        ...
        cmd = ' '.join([
            f'cd {INTERNAL_PROJECT_DIR} && exec {target_dir(self.release)}/add_test_block',
            f'--ledger-db {self.ledger_db_path}',
            f'--watcher-db {self.watcher_db_path}',
...
class FogConformanceTest:
    # Build the internal and public repos for needed code, in release mode
    def build(args):
        ...
        log_and_run_shell(f"cd {INTERNAL_PROJECT_DIR} && CONSENSUS_ENCLAVE_PRIVKEY={enclave_pem} INGEST_ENCLAVE_PRIVKEY={enclave_pem} LEDGER_ENCLAVE_PRIVKEY={enclave_pem} VIEW_ENCLAVE_PRIVKEY={enclave_pem} exec cargo build {FLAGS}")
    ...
    def run(self):
        ...
        # Get chain and key
        root = subprocess.check_output(f"public/{target_dir(self.release)}/mc-crypto-x509-test-vectors --type=chain --test-name=ok_rsa_head",
                                   encoding='utf8', shell=True).strip()
        chain = subprocess.check_output(f"public/{target_dir(self.release)}/mc-crypto-x509-test-vectors --type=chain --test-name=ok_rsa_chain_25519_leaf",
                                   encoding='utf8', shell=True).strip()
        key = subprocess.check_output(f"public/{target_dir(self.release)}/mc-crypto-x509-test-vectors --type=key --test-name=ok_rsa_chain_25519_leaf",
                                 encoding='utf8', shell=True).strip()
        ...
        fog_pubkey = subprocess.check_output(f"cd {INTERNAL_PROJECT_DIR} && exec {target_dir(self.release)}/fog-report-cli --public-address {keyfile} --retry-seconds={FOG_REPORT_RETRY_SECONDS}", shell = True).decode("utf-8")
